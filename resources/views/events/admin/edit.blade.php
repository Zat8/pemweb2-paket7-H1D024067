    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('events.admin.update', $event) }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="quota" value="{{ old('quota', $event->quota) }}" min="1" max="1000"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @error('quota')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                    ...
                    <!-- Upload Poster -->
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Poster Event</label>
                        
                        @if($event->poster)
                            <div class="mt-2 mb-2">
                                <p class="text-xs text-gray-500">Poster saat ini:</p>
                                <img src="{{ Storage::url($event->poster) }}" class="w-48 h-auto rounded border mt-1">
                            </div>
                        @endif

                        <input type="file" name="poster" accept="image/*" 
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        <p class="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah poster. Maks 2MB.</p>
                        @error('poster')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
